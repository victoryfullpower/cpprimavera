    ...
    const exportarExcel = () => {
        try {
            ...
            const datosExcel = datos.map(recibo => ({
                ...
                'Fecha': new Date(recibo.fecharecibo_egreso).toLocaleDateString(),
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'Recibos de Egreso')
            ...
            datos.forEach(recibo => {
                recibo.detalles.forEach(detalle => {
                    detallesData.push({
                        'N° Recibo': recibo.numerorecibo_egreso,
                        'Fecha': new Date(recibo.fecharecibo_egreso).toLocaleDateString(),
                        'Concepto': conceptos.find(c => c.idconcepto_egreso === detalle.idconcepto_egreso)?.descripcion || 'N/A',
            ...
            if (filtros.fechaDesde || filtros.fechaHasta || filtros.idconcepto_egreso || filtros.estado !== '') {
                ...
                if (filtros.idconcepto_egreso) {
                    const concepto = conceptos.find(c => c.idconcepto_egreso === parseInt(filtros.idconcepto_egreso))
                    resumenData.push({ 'Concepto': 'Concepto de egreso', 'Valor': concepto?.descripcion || 'N/A' })
                }
                if (filtros.estado !== '') {
            ...
            const nombreArchivo = `Reporte_Egresos_Conceptos_${new Date().toISOString().split('T')[0]}.xlsx`
